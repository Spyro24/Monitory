...
namespace Monitory_Server_Windows
{
    ...
    class Program
    {
        ...
        private static bool _cpuMultiplirRunning = false;
        private static bool _cpuMhzRunning = false;

        private static double clockSpeedtotalMultiplier = 1.0;
        private static Vector3 clockSpeedTotal = new Vector3(-1f, -1f, -1f);
        ...
        static string CollectData()
        {
            ...
            try
            {
                if (!_cpuMultiplirRunning)
                {
                    _cpuMultiplirRunning = true;
                        ...
                        try
                        {
                            ...
                            using (ManagementObject mo =
                                   new ManagementObject(
                                       "Win32_PerfFormattedData_Counters_ProcessorInformation.Name='_Total'"))
                            {
                                multiplier = (UInt64)(mo["PercentProcessorPerformance"]);
                            }

                            clockSpeedtotalMultiplier = (double)(multiplier) * 0.01;
                        }
                        catch (Exception e)
                        {
                            Console.WriteLine(e);
                ...
                if (!_cpuMhzRunning)
                {
                    ...
                    Thread thread = new Thread(() =>
                    {
                        try
                        {
                            ...
                            {
                                clockSpeedTotal.X = (uint)(mo["MaxClockSpeed"]) * (float)clockSpeedtotalMultiplier;
            ...
            {
                string power =
                    $"Wattage:{powerValue.Key}:{powerValue.Value.X}:{powerValue.Value.Y}:{powerValue.Value.Z}|";
                properties += power;
            }
            ...
            {
                string temp =
                    $"Temperature:{tempValue.Key}:{tempValue.Value.X}:{tempValue.Value.Y}:{tempValue.Value.Z}|";
                properties += temp;
            }
            ...
            {
                string download =
                    $"Download_Speed:{downValue.Key}:{downValue.Value.X}:{downValue.Value.Y}:{downValue.Value.Z}|";
                properties += download;
            }
        ...
        public static void CollectCpuLoadAndClockData(ISensor sensor, ref int threadLoadIndex, ref int threadClockIndex,
            ref string properties)
        {
            ...
            if (sensor.SensorType == SensorType.Load)
            {
                if (sensorName.Contains(TotalName) ||
                    (!sensorName.Contains(CoreName) && !sensorName.Contains(ThreadName)))
                {
        ...
        public static void CollectMemoryData(ISensor sensor, ref string properties)
        {
            ...
            {
                string sensorName = sensor.Name.ToLower();
                if ((sensorName.Contains(AvailableName) || sensorName.Contains(FreeName)) &&
                    !sensorName.Contains(VirtualName))
                {
        ...
        public static void CollectGpuData(ISensor sensor, ref string properties)
        {
            ...
            if (sensor.SensorType == SensorType.Data || sensor.SensorType == SensorType.SmallData)
            {
                ...
                {
                    string line = sensor.SensorType == SensorType.SmallData
                        ? $"Gpu_Memory:Available:{sensor.Value / 1000}:{sensor.Min / 1000}:{sensor.Max / 1000}|"
                        : $"Gpu_Memory:Available:{sensor.Value}:{sensor.Min}:{sensor.Max}|";
                    properties += line;
                }
                ...
                {
                    string line = sensor.SensorType == SensorType.SmallData
                        ? $"Gpu_Memory:Used:{sensor.Value / 1000}:{sensor.Min / 1000}:{sensor.Max / 1000}|"
                        : $"Gpu_Memory:Used:{sensor.Value}:{sensor.Min}:{sensor.Max}|";
                    properties += line;
                }
            }
        }

        public static void CollectTemperatureData(ISensor sensor, string hardwareName,
            ref Dictionary<string, Vector3> temps)
        {
            string sensorName = sensor.Name.ToLower();
        ...
        public static void CollectNetworkData(ISensor sensor, string hardwareName, ref string properties,
            ref Dictionary<string, Vector3> down, ref Dictionary<string, Vector3> up)
        {
            string sensorName = sensor.Name.ToLower();
