...
class Graph:
    def __init__(self, screen_p_x, screen_p_y, size_p_x, size_p_y, raster_p):
        # Origin of the plot
    ...
    def build(self, current_data, screen, average_value_color, bottom_line_color, line_color, fill_color):
        ...
        self.num_slices_x = int(math.floor(self.size_p_x / self.raster_p))
        ...
        for elem_x in self.data_slice:
            ...
            for elem_y in elem_x:
                try:
                    ...
                    continue
                x = (w * self.screen_p_x) - (idx_x * (w * self.raster_p))
                y = (h * self.screen_p_y) - (elem_y * (h * self.size_p_y))
                
                pre_x = (w * self.screen_p_x) - ((idx_x - 1) * (w * self.raster_p))
                ...
                if fill_color[3] > 0 and not filled_quad:
                    filled_quad = True
                    ...
                    if fill_color[3] != 100:
                        ...
                        pygame.draw.polygon(screen, fill_color, polygon_list)
                        # |  \
                        # | __ \
                        ...
                        # pygame.draw.line(screen, fill_color, (pre_x, pre_y), (pre_x, poly_y))
                        # |  \
                        # | __ \
                        polygon_list = [(x, y), (x, poly_y), (pre_x, poly_y)]
                        pygame.draw.polygon(screen, fill_color, polygon_list)
        ...
        # Darw average line
        if average_value_color[3] > 0: # If we have alpha
            average_value = -1.0
            for elem in current_data:
                if average_value < -0.5:
                    ...
                    try:
                        ...
                        continue
            a_x = (w * self.screen_p_x) - ((self.num_slices_x + 2) * (w * self.raster_p))
            a_y = (h * self.screen_p_y) - (average_value * (h * self.size_p_y))
            
            a_pre_x = (w * self.screen_p_x) - ((-2) * (w * self.raster_p))
            # a_pre_y = (h * self.screen_p_y) - (self.data_slice[idx_x - 1][idx_y] * (h * self.size_p_y))
            pygame.draw.line(screen, average_value_color, (a_pre_x, a_y), (a_x, a_y), 2)
        
        # Draw bottom line
        if bottom_line_color[3] > 0: # If we have alpha
            a_x = (w * self.screen_p_x) - ((self.num_slices_x) * (w * self.raster_p))
            a_y = (h * self.screen_p_y)
            
            a_pre_x = (w * self.screen_p_x) - (w * self.raster_p)
            pygame.draw.line(screen, bottom_line_color, (a_pre_x, a_y), (a_x, a_y))
            
class Plot:
    def __init__(self, screen_p_x, screen_p_y, size_p_x, size_p_y, hw_name, hw_name_font_color,\
                hw_name_bg_color, label_font, raster_p=0.005):
        self.screen_p_x = screen_p_x
        self.screen_p_y = screen_p_y
        self.size_p_x = size_p_x
        self.size_p_y = size_p_y
        ...
        self.main_graph = Graph(screen_p_x=screen_p_x, screen_p_y=screen_p_y, \
                                size_p_x=size_p_x, size_p_y=size_p_y, raster_p=self.raster_p)
        
        self.hw_label_text = label_font.render(hw_name, True, hw_name_font_color, hw_name_bg_color)
        self.hw_label_rect = self.hw_label_text.get_rect()
        
        self.stats_label_text = label_font.render(self.stats_name, True, hw_name_font_color, hw_name_bg_color)
        self.stats_label_rect = self.stats_label_text.get_rect()
        
    def update_val(self, perc_val, value, hw_name_font_color, hw_name_bg_color, label_font):
        ...
        self.stats_label_text = label_font.render(wanted_txt, True, hw_name_font_color, hw_name_bg_color)
        self.stats_label_rect = self.stats_label_text.get_rect()
        
                                
    def build(self, screen, graph_data, graph_override_line_color, graph_average_value_color,\
                    graph_bottom_line_color, graph_fill_color):
        self.main_graph.build(graph_data, screen, graph_average_value_color, \
                                graph_bottom_line_color, graph_override_line_color, graph_fill_color)
    ...
    def move_text(self, screen):
        ...
        hw_x = (w * self.screen_p_x) - ((self.num_slices_x) * (w * self.raster_p))
